...
def create_employee_check_out(check_out: CheckOutCreate, db: Session = Depends(get_db)):
...
def list_employee_check_outs(user_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
