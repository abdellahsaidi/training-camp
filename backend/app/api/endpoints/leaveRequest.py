...
def list_leave_requests(employee_id: int = None, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    if employee_id:
...
def read_leave_request(leave_id: int, db: Session = Depends(get_db)):
...
def create_new_leave_request(leave_request: LeaveRequestCreate, db: Session = Depends(get_db)):
...
def update_existing_leave_request(leave_id: int, leave_update: LeaveRequestUpdate, db: Session = Depends(get_db)):
    return update_leave_request(db, leave_id, leave_update)

@router.delete("/{leave_id}")
def delete_existing_leave_request(leave_id: int, db: Session = Depends(get_db)):
